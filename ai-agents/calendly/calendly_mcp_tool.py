...
_CALENDLY_MCP_URL = "https://mcp.calendly.com"


class CalendlyMCPTool(ConnectedServiceToolAgent):
    """
    ConnectedServiceToolAgent wrapper for the Calendly hosted MCP server.

    Connects via SSE/HTTP to https://mcp.calendly.com/mcp using the OAuth 2.1
    Bearer token from the connected service credential. Token refresh is
    delegated to MCPAgentTool via connected_service_id.
    """

    TOOL_DESCRIPTION = (
        "Calendly Agent: manage scheduling with Calendly — list event types, check availability, "
        "create and share scheduling links, book meetings, cancel events, and generate one-time "
        "booking links using natural language."
    )
    ...
    def _build_mcp_config(self) -> Dict[str, Any]:
        ...
        logger.info(
            "Calendly MCP config build: connected_service_id=%s token_present=%s",
            cs_id,
        ...
        mcp_config: Dict[str, Any] = {
            "server_type": "sse",
            "url": active.get("mcp_url") or _CALENDLY_MCP_URL,
            "headers": headers,
            "command": None,
            "args": [],
            "env": {},
    ...
    async def initialize(self) -> None:
        # If ConnectedServiceToolAgent found no token (Calendly plugin sends connected_service_id
        ...
        if not self._access_token:
            ...
            if cs_id:
                ...
                logger.info("Calendly pre-init token refresh: cs_id=%s success=%s", cs_id, refreshed)
