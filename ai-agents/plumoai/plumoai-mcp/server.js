...
const server = new FastMCP({
    ...
    authenticate: async (request) => {
        // HTTP transport provides an IncomingMessage with headers.
        // STDIO transport does not, so we support env-based auth for stdio.
        const headerFromHttp = request?.headers?.authorization;
        const envToken = process.env.PLUMO_ACCESS_TOKEN?.trim();
        const envCompanyId = process.env.PLUMO_COMPANY_ID?.trim();
        let authHeader = headerFromHttp ??
            (envToken ? (envToken.startsWith("Bearer ") ? envToken : `Bearer ${envToken}`) : undefined);
        let companyId = envCompanyId && envCompanyId.length > 0 ? envCompanyId : null;
        ...
        try {
            const response = await axios.get(`https://api.plumoai.com/Auth/oauth/me`, {
                headers: {
            ...
            if (companyId == null || String(companyId).trim().length === 0) {
                throw new Response(null, {
                    status: 401,
                    statusText: "Missing Company ID",
                });
            }
            if (response.data.data.companyIds.map((x) => x.toString()).indexOf(String(companyId)) < 0) {
                throw new Response(null, {
                    status: 401,
            ...
            return {
                user_access_token: authHeader.slice(7),
                expires_in: 900,
...
server.addTool({
    ...
    execute: async (args, { session }) => {
        ...
        var responseData = await getSpService().storedProcedureComplianceDb(data, session?.companyId, {
            isOpenApi: false,
...
server.addTool({
    ...
    execute: async (args, { session }) => {
        try {
            ...
            const responseData = await getSpService().storedProcedureComplianceDb(data, session?.companyId, { isOpenApi: false }, String(session?.user_access_token ?? ""));
...
server.addTool({
    ...
    execute: async (args, { session }) => {
        let projResponse = await getSpService().storedProcedureComplianceDb({
...
async function documentsList(documentId, projectId, userId, companyId, user_access_token) {
    ...
    const responseData = await getSpService().storedProcedureComplianceDb(data, companyId, { isOpenApi: false }, String(user_access_token ?? ""));
